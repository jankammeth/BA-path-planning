...
def plot_runtime_boxplot(
    ...
    if show_means and 'means' in bp:
        for mean in bp['means']:
            mean.set(marker='o', markersize=5, markerfacecolor='black', markeredgecolor='white', linestyle='None', zorder=4)

    # Whisker fliers (outliers)
    if 'fliers' in bp:
        for flier in bp['fliers']:
            flier.set(marker='o', alpha=0.35, markersize=5, markerfacecolor='none', markeredgecolor='black')

    # X tick labels in LaTeX
    ax.set_xticks(range(1, len(Ns) + 1))
    ax.set_xticklabels([fr"$N={N}$" for N in Ns])
...
def main():
    ...
    parser.add_argument("--title", type=str, default="SCP Computation Time vs Number of Robots", help="Plot title.")
