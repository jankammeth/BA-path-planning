...
# ---------- Initial edge clusters (300×100) ----------
def generate_edge_cluster_positions(n_vehicles, min_distance=1.0, rng=None):
    if rng is None:
        rng = np.random.default_rng()
    # Spread clusters along the 300x100 box: corners + midpoints
    centers = np.array([
        [10, 10],   [150, 10],   [290, 10],   # bottom
        [10, 50],                  [290, 50], # middle sides
        [10, 90],   [150, 90],   [290, 90],   # top
    ], float)
    ...
    pts, cluster_ids = [], []
    cov = np.array([[8.0, 0.0], [0.0, 4.0]])  # slightly elliptical spread
...
# ---------- SVG outline sampler ----------
def svg_outline_points(svg_file, n_points, box_size=(300, 100),
...
def plot_mapping(initial, logo_pts, colors):
    ...
    plt.tight_layout()
    plt.savefig("clusters_to_ETHZ_logo_300x100.png",
...
def main():
    ...
    logo = svg_outline_points("ETHZ.svg", N, box_size=(300, 100),
                              target_width_ratio=0.9, y_center_ratio=0.5)
