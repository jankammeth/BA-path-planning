...
# Your modules
from scp import SCP
from position_generator import generate_positions  # (initial_positions, final_positions)
...
def run_single_trial(N, cfg, rng):
    ...
    # Generate positions (you can mutate generator to accept rng if needed)
    init_pos, final_pos = generate_positions(N, 1.0)#cfg["min_distance"])
    # print_distance_analysis(init_pos, final_pos)  # optional

    planner.set_initial_states(init_pos)
    planner.set_final_states(final_pos)

    t0 = time.perf_counter()
    status = "success"
    err_msg = None
    try:
        _ = planner.generate_trajectories(max_iterations=cfg["max_iterations"])
    ...
    result = {
        ...
        "T": getattr(planner, "T", cfg["time_horizon"]),
        "h": getattr(planner, "h", cfg["time_step"]),
...
def main():
    ...
    print(f"T={cfg['time_horizon']}s, h={cfg['time_step']}s, R={cfg['min_distance']}m, space={cfg['space_dims']}")
    ...
    for N in cfg["Ns"]:
        ...
        for trial in range(cfg["trials_per_N"]):
            ...
            print(f"  trial {trial+1:02d}/{cfg['trials_per_N']}  time = {res['time_sec']:.3f}s  [{status_str}]")

        print()

    # Build summary stats for quick inspection / boxplots later
    for N in cfg["Ns"]:
        times = [r["time_sec"] for r in all_results["runs"] if r["N"] == N and r["status"] == "success"]
        ...
        if times:
            ...
            all_results["summary"][str(N)] = {
                ...
                "min": None, "max": None, "mean": None, "median": None, "p25": None, "p75": None, "std": None,
            }
    ...
    for N in cfg["Ns"]:
        ...
        print(f"  N={N}: count={s['count']}, errors={s['errors']}, "
